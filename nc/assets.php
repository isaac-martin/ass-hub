...
function asshub_assets_header() {
  ...
  $file = 'x-header';
  wp_enqueue_script( 'asshub-header', get_stylesheet_directory_uri() . '/js/' . $file . '.js', $deps = array('jquery'), filemtime( get_template_directory() . '/js/' . $file . '.js' ), false );

  // Register and enqueue our main stylesheet with versioning based on last modified time
  wp_register_style( 'asshub-style', get_stylesheet_uri(), $dependencies = array(), filemtime( get_template_directory() . '/style.css' ) );
  wp_enqueue_style( 'asshub-style' );
}
add_action( 'wp_enqueue_scripts', 'asshub_assets_header' );



// Load footer assets; a more complex example of a smooth asset-loading approach for WordPress themes
function asshub_assets_footer() {

  // Initialize variables
  $name = 'asshub-footer';   // This is the script handle
  ...
  if ( asshub_SCRIPTS_PAGELOAD && ( is_archive() || is_home() || is_search() ) ) {
...
add_action( 'wp_enqueue_scripts', 'asshub_assets_singular' );
